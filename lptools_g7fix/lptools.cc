...
int main(int argc, char **argv) {
    ...
    if(strcmp(argv[1], "create") == 0) {
        ...
        // g7tabpro fix: findGroup() above always resolves to the group that
        // holds the CURRENTLY BOOTED slot's own "system" partition (it keys
        // off ro.boot.slot_suffix, not off partName). That's correct when
        // creating a same-slot partition but silently wrong for cross-slot
        // creates (e.g. running "create system_b ..." while booted on _a
        // lands system_b in group main_a instead of main_b). Group names on
        // this device are the base name plus the slot suffix (main_a /
        // main_b), so rederive the group for partName's own suffix and use
        // it if it actually exists; otherwise fall back to the original
        // heuristic unchanged.
        std::string createGroup = group;
        {
            std::string curSuffix = ::android::base::GetProperty("ro.boot.slot_suffix", "");
            std::string targetSuffix = GetPartitionSlotSuffix(partName);
            if (!targetSuffix.empty() && targetSuffix != curSuffix &&
                group.size() > curSuffix.size() &&
                group.compare(group.size() - curSuffix.size(), curSuffix.size(), curSuffix) == 0) {
                std::string candidate = group.substr(0, group.size() - curSuffix.size()) + targetSuffix;
                if (builder->FindGroup(candidate) != nullptr) {
                    std::cerr << "Cross-slot create detected for " << partName
                              << ": using group " << candidate << " instead of " << group << std::endl;
                    createGroup = candidate;
                }
            }
        }

        partition = builder->AddPartition(partName, createGroup, 0);
        if(partition == nullptr) {
